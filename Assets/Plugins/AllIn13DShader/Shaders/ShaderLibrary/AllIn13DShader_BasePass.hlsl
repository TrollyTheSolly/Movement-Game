...
float4 BasicFragment(FragmentData i) : SV_Target
{
	...
	float3 lightmap = GetLightmap(UV_LIGHTMAP(i));
	...
	float3 subtractedLight = lightmap - shadowedLightEstimate;
	subtractedLight = max(subtractedLight, unity_ShadowColor.rgb);
	subtractedLight = lerp(subtractedLight, lightmap, _LightShadowData.x);
	
	lightmap = subtractedLight;
#endif
